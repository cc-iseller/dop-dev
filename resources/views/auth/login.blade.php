...
<html lang="id">
  <head>
    ...
    <title>Login — iseller</title>
    <meta name="description" content="Halaman login iseller (Laravel + Filament) menggunakan Tailwind CSS." />
  ...
  <body class="min-h-screen bg-slate-950 text-slate-100">
    ...
    <div class="pointer-events-none fixed inset-0 overflow-hidden">
      ...
      ></div>
      <div
        class="absolute top-40 -left-40 h-[520px] w-[520px] rounded-full bg-cyan-500/10 blur-3xl"
      ></div>
    </div>

    <main class="relative z-10 mx-auto flex max-w-7xl items-center justify-center px-6 py-14">
      <div class="w-full max-w-md rounded-2xl bg-white/5 p-6 ring-1 ring-white/10">
        ...
        <h1 class="text-2xl font-bold tracking-tight text-white">Login</h1>
        <p class="mt-1 text-sm text-slate-300">Masuk dengan akun yang sudah terdaftar.</p>

        {{-- Alert error dari Laravel --}}
        @if ($errors->any())
          <div class="mt-4 rounded-xl bg-red-500/10 p-4 text-sm text-red-100 ring-1 ring-red-400/20">
            <div class="mb-2 font-semibold">Login gagal:</div>
        ...
        @if (session('status'))
          <div class="mt-4 rounded-xl bg-indigo-500/10 p-4 text-sm text-indigo-100 ring-1 ring-indigo-400/20">
            {{ session('status') }}
          </div>
        @endif
        ...
        >
          ...
          <div>
            ...
            />
            <p class="mt-1 text-xs text-slate-400">Format email harus valid (nama@domain.com).</p>
          </div>
          ...
          <div class="flex items-center justify-between">
            ...
            <a href="#" class="text-xs text-indigo-200 hover:text-indigo-100">
              Lupa password?
            </a>
          </div>
          ...
          <p class="pt-2 text-center text-xs text-slate-400">
            Belum punya akun?
            <a href="/register" class="font-semibold text-indigo-300 hover:text-indigo-200">Daftar sekarang</a>
          </p>
        </form>

        <p class="mt-6 text-center text-xs text-slate-400">© <span id="year"></span> iseller</p>
      </div>
    </main>
